...
export const addReferals = async (req, res, next) => {
  try {
  
    const listing = await Referals.create({ ...req.body });
...
export const getDoc = async (req, res) => {
  try {
    // Retrieve all Referrals data from the database
    const referals = await Referals.find();

    // Construct response object
    const responseData = referals.map(referal => ({
      _id: referal._id,
      name: referal.name,
      contactNumber: referal.contactNumber,
      alternativeContact: referal.alternativeContact,
      state: referal.state,
      city: referal.city,
      pin: referal.pin,
      district: referal.district,
      pan: referal.pan,
      aadhar: referal.aadhar,
      code: referal.code,
      userRef: referal.userRef,
